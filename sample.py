...
class Classifier(object):
    def __init__(self, path="",features_train = None, labels_train = None, features_test = None, labels_test = None,  estimator = LinearSVC(random_state=4)):
        ...
        self.features_train_scaled = self.scaler.fit_transform(features_train)
        self.features_test_scaled = self.scaler.transform(features_test)
        self.path_no_scale = path + "noscale.txt"
        self.path_scale = path + "scale.txt"

    def training_no_sclale(self):
        self.estimator.fit(self.features_train, self.labels_train)
        self.__training_result()

    def training_sclale(self):
        self.estimator.fit(self.features_train_scaled, self.labels_train)
        self.__training_result_scale()

    def __training_result(self):
        fileNoScale = open(self.path_no_scale,"w")
        y_true, y_pred = self.labels_test, self.estimator.predict(self.features_test)
        fileNoScale.write(classification_report(y_true, y_pred))
        fileNoScale.write("F1: " + str(f1_score(y_true, y_pred, average='weighted')) + "\n")
        fileNoScale.write("Acc: " + str(accuracy_score(y_true, y_pred)))


    def __training_result_scale(self):
        fileScale = open(self.path_scale, "w")
        y_true, y_pred = self.labels_test, self.estimator.predict(self.features_test_scaled)
        fileScale.write(classification_report(y_true, y_pred))
        fileScale.write("F1: " + str(f1_score(y_true, y_pred,  average='weighted')) + "\n")
        fileScale.write("Acc: " + str(accuracy_score(y_true, y_pred)))
...
if __name__ == '__main__':
    ...
    svc = Classifier(path=settings.SVM_NOSCALE, features_train=features_train, features_test=features_test, labels_train=labels_train, labels_test=labels_test)
    svc.training_no_sclale()
    svc.training_sclale()
    knn = Classifier(path=settings.KNN_NOSCALE,features_train=features_train, features_test=features_test, labels_train=labels_train, labels_test=labels_test, estimator=KNeighborsClassifier(n_neighbors=9))
    knn.training_no_sclale()
    knn.training_sclale()
    nb = Classifier(path=settings.NB_NOSCALE,features_train=features_train, features_test=features_test, labels_train=labels_train, labels_test=labels_test, estimator=GaussianNB())
    nb.training_no_sclale()
    nb.training_sclale()